...
def track_and_remove(tracker, rgb, points, var_threshold=10.):
    B, T, C, H, W = rgb.shape
    pred_tracks, pred_vis = tracker(rgb, queries=points, backward_tracking=True) # [1, T, N, 2]

    var = torch.var(pred_tracks, dim=1)  # [1, N, 2]
    var = torch.sum(var, dim=-1)[0]  # List

    # vis.visualize(video=rgb, tracks=pred_tracks, visibility=pred_vis, filename='test_remove')
    ...
    # Track new points
    pred_tracks, pred_vis = tracker(rgb, queries=new_points, backward_tracking=True)
    # vis.visualize(video=rgb, tracks=pred_tracks, visibility=pred_vis, filename='test_remove')
    var = torch.var(pred_tracks, dim=1)  # [1, N, 2]
    var = torch.sum(var, dim=-1)[0]  # List

    # get top 800 points with high variance
    idx = torch.argsort(var, descending=True)[:800]
        ...
        if self.tracks_leave_trace != 0:
            for t in range(1, T):
                if future:
                    first_ind = 0
                    end_ind = (
                        min(T, t + self.tracks_leave_trace)
                        if self.tracks_leave_trace >= 0
                        else T
                    )
                    curr_tracks = tracks[t : end_ind]
                    curr_colors = vector_colors[t : end_ind]
                    alpha = 0.2
                else:
                    first_ind = (
                    ...
                    end_ind = t
                    curr_tracks = tracks[first_ind : end_ind + 1]
                    curr_colors = vector_colors[first_ind : end_ind + 1]
                    alpha = 0.5
                if compensate_for_camera_motion:
                    diff = (
